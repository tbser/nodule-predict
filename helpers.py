...
def getlogger(log_name):
    ...
    if not len(mylogger.handlers): 
        ...
        filehandler = logging.FileHandler(log_name + '.log')
...
def rescale_patient_images(images_zyx, org_spacing_xyz, target_voxel_mm, is_mask_image=False, verbose=False, logger=None):
    if verbose:
        print("Spacing: ", org_spacing_xyz)
        print("Shape: ", images_zyx.shape)

    if logger is not None:
        logger.info("Resizing dim z")
    resize_x = 1.0
    ...
    if logger is not None:
        logger.info("After resizing dim z, Shape is now : {0}".format(res.shape))

    res = res.swapaxes(0, 2)
    res = res.swapaxes(0, 1)
    if logger is not None:
        logger.info("After swap axies (0, 2) (0, 1), Shape is now : {0}".format(res.shape))
    ...
    if logger is not None:
        logger.info("Final Shape is now : {0} ".format(res.shape))
    return res
