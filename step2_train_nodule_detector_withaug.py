...
sys.stderr.flush()
logfilehandle = open("./train_with_data_aug.txt", 'a+')
os.dup2(logfilehandle.fileno(), sys.stderr.fileno())
# limit memory usage..
...
POS_WEIGHT = 2
NEGS_PER_POS = 2   # ============== NEGS_PER_POS = 20
P_TH = 0.6
...
def get_train_holdout_files(fold_count, train_percentage=80, logreg=True, ndsb3_holdout=0, manual_labels=True, full_luna_set=False, local_patient_set=False):
    ...
    logger.info("Pos samples lidc: {0}".format(len(pos_samples_lidc)))
    ...
    logger.info("Original pos samples: {0}".format(len(pos_samples_original)))  
    ...
    logger.info("Augmented pos samples: {0}".format(len(pos_samples_aug)))  
    ...
    # pos_samples_train = pos_samples_aug
    # pos_samples_holdout = pos_samples_original
    ...
    logger.info("Neg samples edge: {0}".format(len(neg_samples_edge)))
    ...
    logger.info("Neg samples luna: {0}".format(len(neg_samples_luna)))
    ...
    logger.info("Neg samples falsepos LUNA count: {0}".format(len(neg_samples_falsepos)))
...
get_net.__annotations__ = {'return': Model}
...
def train(model_name, fold_count, train_full_set=False, load_weights_path=None, ndsb3_holdout=0, manual_labels=True,
    ...
    logger.info("get_train_holdout_files is done.")
    ...
    train_history = model.fit_generator(train_gen, len(train_files) / batch_size, 12, validation_data=holdout_gen,
                                        validation_steps=len(holdout_files) / batch_size,
                                        callbacks=[logcallback, tensorboard_callback, checkpoint, checkpoint_fixed_name, learnrate_scheduler])
