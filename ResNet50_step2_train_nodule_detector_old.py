...
from keras.callbacks import ModelCheckpoint, Callback, LearningRateScheduler, History
import shutil
...
def train(model_name, fold_count, train_full_set=False, load_weights_path=None, ndsb3_holdout=0, manual_labels=True):
    ...
    model = ResNet50.get_resnet50(input_shape=(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE, 1), load_weight_path=model_name)
    ...
    train_history = model.fit_generator(train_gen, len(train_files) / batch_size, 100, validation_data=holdout_gen,
                                        ...
                                        callbacks=[logcallback, checkpoint, checkpoint_fixed_name, learnrate_scheduler])
