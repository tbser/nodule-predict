...
def work(d, f, op_lists, dst_dir):
    try:
        ...
        for op_list in op_lists:
            ...
            for op in op_list:
                img = op.process(img)
            imsave(os.path.join(dst_dir, out_file_name), img)
...
def process(dir, file, op_lists, dst_dir):
    thread_pool.apply_async(work, (dir, file, op_lists, dst_dir))
