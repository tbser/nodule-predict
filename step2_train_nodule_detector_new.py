...
import shutil
from timeprofile import calltimeprofile
...
def train(model_name, fold_count, train_full_set=False, load_weights_path=None, ndsb3_holdout=0, manual_labels=True, local_patient_set=False):
    ...
    model_name = model_name + holdout_txt

    model = ThreeDCNN(logger)
    model.generate_model((CUBE_SIZE, CUBE_SIZE, CUBE_SIZE, 1), dropout=True, load_weight_path=load_weights_path)
    model.train(model_name, train_gen, len(train_files), holdout_gen,
                len(holdout_files), 12, settings.WORKING_DIR + "workdir/", batch_size)
