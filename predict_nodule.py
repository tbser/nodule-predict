    ...
    for index, row in df_nodule_predictions.iterrows():
        ...
        if not nodule_in_mask:
            ...
            if (z_perc > 0.75 or z_perc < 0.25) and y_perc > 0.85:
                logger.info("SUSPICIOUS FALSEPOSITIVE: {0}  center z: {1}  y_perc: {2}".format(patient_id, center_z, y_perc))

            if center_z < 50 and y_perc < 0.30:
                logger.info("SUSPICIOUS FALSEPOSITIVE OUT OF RANGE: {0} center z: {1} y_perc: {2}".format(patient_id, center_z, y_perc))
...
def predict_cubes(model_path, continue_job, data_source = "ndsb3", only_patient_id=None, magnification=1, flip=False, train_data=True, holdout_no=-1, ext_name="", fold_count=2):
    logger.info("Predict cubes with model {0}, magnification {1}, train_data {2}, data_source {3} ".format(model_path, magnification, train_data, data_source))
    if data_source == "luna16":
        dst_dir = settings.LUNA_NODULE_DETECTION_DIR
        src_dir = settings.LUNA16_EXTRACTED_IMAGE_DIR
    elif data_source == "ndsb3":
        dst_dir = settings.NDSB3_NODULE_DETECTION_DIR
        src_dir = settings.NDSB3_EXTRACTED_IMAGE_DIR
    else:
        dst_dir = settings.HOSPITAL_NODULE_DETECTION_DIR
        src_dir = settings.HOSPITAL_EXTRACTED_IMAGE_DIR
    ...
    for file_name in os.listdir(src_dir):
        if not os.path.isdir(src_dir + file_name):
            continue
    ...
    for patient_index, patient_id in enumerate(reversed(patient_ids)):
        ...
        logger.info("Predict volume shape: {0}".format( predict_volume.shape))
        done_count = 0
        skipped_count = 0
        batch_size = 128
        batch_list = []
            ...
            for y in range(0, predict_volume_shape[1]):
                for x in range(0, predict_volume_shape[2]):
                    ...
                    if cube_mask.sum() < 2000:
                        skipped_count += 1
                        #logger.info("Cube x {0} y  {1} z {2} is skipped!!!".format(x, y, z))
                    else:
                        if flip:
                            cube_img = cube_img[:, :, ::-1]
                        ...
                        if len(batch_list) % batch_size == 0:
                            ...
                            for i in range(len(p[0])):
                                ...
                                if nodule_chance > P_TH:
                                    logger.info("Cube x {0} y  {1} z {2} is possible nodule, nodule_chance is {3}!!!".format(p_x, p_y, p_z,nodule_chance))
                                    p_z = p_z * step + CROP_SIZE / 2
                                    p_y = p_y * step + CROP_SIZE / 2
                                    p_x = p_x * step + CROP_SIZE / 2

                                    logger.info("Cube center x {0} y {1} z {2} ".format(p_x, p_y, p_z))
                                    ...
                                    logger.info("Cube center percentage x {0} y {1} z {2} diamm {3} ".format(p_x_perc, p_y_perc, p_z_perc,diameter_mm))
                    ...
                    if done_count % 10000 == 0:
                        logger.info("Done: {0} skipped: {1}".format( done_count,skipped_count))
        ...
        filter_patient_nodules_predictions(df, patient_id, CROP_SIZE * magnification, data_source)
        ...
        logger.info("predict_volume mean is {0}".format(predict_volume.mean()))
        logger.info("Done in {0} seconds".format(sw.get_elapsed_seconds()))
    ...
    try:
        if True:
            for magnification in [1, 1.5, 2]:  #
                predict_cubes("models/model_luna16_full__fs_best.hd5", CONTINUE_JOB, "hospital", only_patient_id, magnification=magnification, flip=False, train_data=True, holdout_no=None, ext_name="luna16_fs")
                predict_cubes("models/model_luna16_full__fs_best.hd5", CONTINUE_JOB, "hospital", only_patient_id=only_patient_id, magnification=magnification, flip=False, train_data=False, holdout_no=None, ext_name="luna16_fs")
    
        if True:
            for version in [2, 1]:
                for holdout in [0, 1]:
                    for magnification in [1, 1.5, 2]:  #
                        predict_cubes("models/model_luna_posnegndsb_v" + str(version) + "__fs_h" + str(holdout) + "_end.hd5", CONTINUE_JOB, "hospital", only_patient_id=only_patient_id, magnification=magnification, flip=False, train_data=True, holdout_no=holdout, ext_name="luna_posnegndsb_v" + str(version), fold_count=2)
                        if holdout == 0:
                            predict_cubes("models/model_luna_posnegndsb_v" + str(version) + "__fs_h" + str(holdout) + "_end.hd5", CONTINUE_JOB, data_source="hospital", only_patient_id=only_patient_id, magnification=magnification, flip=False, train_data=False, holdout_no=holdout, ext_name="luna_posnegndsb_v" + str(version), fold_count=2)
