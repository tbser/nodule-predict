...
def train_model(holdout, model_type, continue_from=None):
    ...
    train_history = model.fit_generator(train_gen, len(train_files) / batch_size, epoch_count, validation_data=holdout_gen,
                        validation_steps=len(holdout_files) / batch_size, callbacks=[checkpoint1, checkpoint2, logcallback])
    ...
    logger.info("Train history keys: {0}".format(train_history.history.keys()))
    pandas.DataFrame(train_history.history).to_csv(
        settings.WORKING_DIR + "workdir/" + model_type +"_model_h" + str(holdout) + "history.csv")
...
def predict_patients(patients_dir, model_path, holdout, patient_predictions, model_type):
    ...
    for item_name in os.listdir(patients_dir):
        ...
        #     continue
        logger.info("Patient ID: {0}".format(patient_id))
