...
def predict(model_path, img_list):
    model = ResNet50.get_resnet50(input_shape=(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE, 1), load_weight_path=model_path)
...
def predict_area(model_path, flip=False, ext_name=""):
    logger.info("Predict cubes with model {0}".format(model_path))
    ...
    predictions = predict(model_path, img_list)
    true_positive, false_negative = 0, 0
    true_negative, false_positive = 0, 0
    area_predictions_csv = []

    for item in predictions:
        file_name, nodule_chance, diameter_mm = item[0], item[1], item[2]
        ...
        if pn == "pos":
            if nodule_chance > P_TH:
                ...
                filter_wrong_predict_file(file_name, settings.WRONG_PREDICTION_FN)
        else:
            if nodule_chance > P_TH:
                false_positive += 1
                result = "false positive"
                filter_wrong_predict_file(file_name, settings.WRONG_PREDICTION_FP)
            else:
                true_negative += 1
...
if __name__ == "__main__":
    try:
        if True:
            ...
            true_positive, false_negative, false_positive, true_negative = predict_area("/opt/data/deeplearning/train_result/ResNet50_separate_train_data/models/model_luna16_full__fs_best.hd5", flip=False, ext_name="luna16_fs")
            ...
            logger.info(
                "特异性specificity = TNR(True Negative Rate) = TN /(FP + TN) = 1 - FPR = {0}".format(true_negative_rate))
