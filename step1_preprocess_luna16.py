...
settings.log = logging.getLogger('stpe1_luna16')
    ...
    for reading_session in reading_sessions:
        ...
        for nodule in nodules:
            ...
            if nodule.characteristics is None:
                settings.log.info("!!!!Nodule: {0} has no charecteristics".format(nodule_id))
                continue
            if nodule.characteristics.malignancy is None:
                settings.log.info("!!!!Nodule: {0} has no malignacy".format(nodule_id))
                continue
...
def process_pos_annotations_patient(src_path, patient_id):
    ...
    settings.log.info("Annos: {0}".format(len(df_patient)))
    ...
    if round(direction[0]) == -1:
        ...
        settings.log.info("Swappint x origin")
    if round(direction[4]) == -1:
        origin[1] *= -1
        direction[4] = 1
        flip_direction_y = True
        settings.log.info("Swappint y origin")
    settings.log.info("Direction: {0}".format(direction))
    ...
    for index, annotation in df_patient.iterrows():
        ...
        settings.log.info("Node org (x,y,z,diam): {0} {1} {2} {3}".format(round(node_x, 2), round(node_y, 2), round(node_z, 2), round(diam_mm, 2)))
        ...
        settings.log.info("Node tra (x,y,z): {0} {1} {2}".format(center_int[0], center_int[1], center_int[2]))
        ...
        # center_int = numpy.rint((center_float - origin) )
        settings.log.info("Node sca (x,y,z):  {0} {1} {2}".format(center_float_rescaled[0], center_float_rescaled[1], center_float_rescaled[2]))
...
def process_excluded_annotations_patient(src_path, patient_id):
    ...
    for index, annotation in df_patient.iterrows():
        ...
        center_float_percent = center_float_rescaled / patient_imgs.swapaxes(0, 2).shape
        # center_int = numpy.rint((center_float - origin) )
        # settings.log.info("Node sca (x,y,z,diam): ", (center_float_rescaled[0], center_float_rescaled[1], center_float_rescaled[2]))
        ...
        for index, row in pos_annos_df.iterrows():
            ...
            settings.log.info("{} {} {}".format(pos_coord_x, pos_coord_y, pos_coord_z))
            settings.log.info("{}".format(center_float_rescaled))
            ...
            if dist < (diameter + 64):  #  make sure we have a big margin
                ok = False
                settings.log.info("################### Too close {}".format(center_float_rescaled))
                break
...
def process_luna_candidates_patient(src_path, patient_id):
    ...
    img_array = SimpleITK.GetArrayFromImage(itk_img)
    settings.log.info("Img array: {0}".format(img_array.shape))
    settings.log.info("Pos annos: {0}".format(len(df_pos_annos)))
    ...
    for df_index, candiate_row in src_df.iterrows():
        ...
        # settings.log.info("Node org (x,y,z,diam): ", (round(node_x, 2), round(node_y, 2), round(node_z, 2), round(candidate_diameter, 2)))
        center_float = numpy.array([node_x, node_y, node_z])
        center_int = numpy.rint((center_float-origin) / spacing)
        # center_int = numpy.rint((center_float - origin) )
        # settings.log.info("Node tra (x,y,z,diam): ", (center_int[0], center_int[1], center_int[2]))
        # center_int_rescaled = numpy.rint(((center_float-origin) / spacing) * rescale)
        center_float_rescaled = (center_float - origin) / settings.TARGET_VOXEL_MM
        center_float_percent = center_float_rescaled / patient_imgs.swapaxes(0, 2).shape
        # center_int = numpy.rint((center_float - origin) )
        ...
        for index, row in df_pos_annos.iterrows():
            ...
            if dist < (diameter + 64):  #  make sure we have a big margin
                ok = False
                settings.log.info("################### Too close {0}".format((coord_x, coord_y, coord_z)))
                break
        ...
        if pos_annos_manual is not None and ok:
            for index, row in pos_annos_manual.iterrows():
                ...
                pos_coord_z = row["z"] * patient_imgs.shape[0]
                diameter = row["d"] * patient_imgs.shape[2]
                settings.log.info("{0} {1} {2}".format(pos_coord_x, pos_coord_y, pos_coord_z))
                settings.log.info("{0}".format(center_float_rescaled))
                dist = math.sqrt(math.pow(pos_coord_x - center_float_rescaled[0], 2) + math.pow(pos_coord_y - center_float_rescaled[1], 2) + math.pow(pos_coord_z - center_float_rescaled[2], 2))
                if dist < (diameter + 72):  #  make sure we have a big margin
                    ok = False
                    settings.log.info("################### Too close {0}".format(center_float_rescaled))
                    break
...
def process_auto_candidates_patient(src_path, patient_id, sample_count=1000, candidate_type="white"):
    ...
    if tries > 9999:
        settings.log.info("****** WARING!! TOO MANY TRIES ************************************")
    df_candidates = pandas.DataFrame(candidate_list, columns=["anno_index", "coord_x", "coord_y", "coord_z", "diameter", "malscore"])
...
def process_images(delete_existing=False, only_process_patient=None):
    if delete_existing and os.path.exists(settings.LUNA16_EXTRACTED_IMAGE_DIR):
        settings.log.info("Removing old stuff..")
    ...
    for subject_no in range(settings.LUNA_SUBSET_START_INDEX, 10):
        ...
        for src_path in glob.glob(src_dir + "*.mhd"):
            if only_patient is not None and only_patient not in src_path:
                continue
            patient_id = os.path.basename(src_path).replace(".mhd", "")
            settings.log.info("Index {0} patient id {1}".format(candidate_index, patient_id))
...
def process_auto_candidates_patients():
    for subject_no in range(settings.LUNA_SUBSET_START_INDEX, 10):
        ...
        for patient_index, src_path in enumerate(glob.glob(src_dir + "*.mhd")):
            ...
            #     continue
            patient_id = os.path.basename(src_path).replace(".mhd", "")
            settings.log.info("Index {0} patient id {1}".format(patient_index, patient_id))
...
def process_lidc_annotations(only_patient=None, agreement_threshold=0):
    ...
    for anno_dir in [d for d in glob.glob("resources/luna16_annotations/*") if os.path.isdir(d)]:
        ...
        for xml_path in xml_paths:
            settings.log.info("file_no {0} xml file {1}".format(file_no, xml_path))
            pos, neg, extended = load_lidc_xml(xml_path=xml_path, only_patient=only_patient, agreement_threshold=agreement_threshold)
            if pos is not None:
                pos_count += len(pos)
                neg_count += len(neg)
                settings.log.info("Pos: {0} Neg: {1}".format(pos_count, neg_count))
                file_no += 1
                all_lines += extended
            # if file_no > 10:
            #     break
    ...
    df_annos.to_csv(settings.WORKING_DIR + "lidc_annotations.csv", index=False)


if __name__ == "__main__":
    stopwatch = helpers.Stopwatch.start_new()
    totalcost = 0
    if True:
        only_process_patient = None
        settings.log.info('start process_images()')
        process_images(delete_existing=False, only_process_patient=only_process_patient)
        funccost = stopwatch.get_elapsed_seconds()
        totalcost = totalcost + funccost
        settings.log.info('process_images cost {0} seconds '.format(funccost))
        input('Press any key to continue...')

    if True:
        stopwatch.start()
        settings.log.info('start process_lidc_annotations()')
        process_lidc_annotations(only_patient=None, agreement_threshold=0)
        funccost = stopwatch.get_elapsed_seconds()
        totalcost = totalcost + funccost
        settings.log.info('process_lidc_annotations cost {0} seconds '.format(funccost))
        input('Press any key to continue...')

    if True:
        stopwatch.start()
        settings.log.info('start process_pos_annotations_patient2()')
        process_pos_annotations_patient2()
        funccost = stopwatch.get_elapsed_seconds()
        totalcost = totalcost + funccost
        settings.log.info('process_pos_annotations_patient2 cost {0} seconds '.format(funccost))
        input('Press any key to continue...')
        stopwatch.start()
        settings.log.info('start process_excluded_annotations_patients()')
        process_excluded_annotations_patients(only_patient=None)
        funccost = stopwatch.get_elapsed_seconds()
        totalcost = totalcost + funccost
        settings.log.info('process_excluded_annotations_patients cost {0} seconds '.format(funccost))
        input('Press any key to continue...')

    if True:
        stopwatch.start()
        settings.log.info('start process_luna_candidates_patients()')
        process_luna_candidates_patients(only_patient_id=None)
        funccost = stopwatch.get_elapsed_seconds()
        totalcost = totalcost + funccost
        settings.log.info('process_luna_candidates_patients cost {0} seconds '.format(funccost))
        input('Press any key to continue...')
    if True:
        stopwatch.start()
        settings.log.info('start process_auto_candidates_patients()')
        process_auto_candidates_patients()
        funccost = stopwatch.get_elapsed_seconds()
        totalcost = totalcost + funccost
        settings.log.info('process_auto_candidates_patients cost {0} seconds '.format(funccost))
        input('Press any key to finish..., the total cost time is {0} seconds'.format(totalcost))
