...
def identity_block(input_tensor, nb_filter, stage, block, kernel_size=3):
    ...
    out = Conv3D(nb_filter1, (1, 1, 1), name=conv_name_base + '2a', data_format="channels_last")(input_tensor)
    ...
    out = Conv3D(nb_filter2, kernel_size, name=conv_name_base + '2b', padding="same", data_format="channels_last")(out)
    out = BatchNormalization(axis=bn_axis, name=bn_name_base + '2b')(out)
    out = Activation('relu')(out)

    out = Conv3D(nb_filter3, (1, 1, 1), name=conv_name_base + '2c', data_format="channels_last")(out)
...
def conv_block(input_tensor, nb_filter, stage, block, kernel_size=3, strides=(2, 2, 2)):
    ...
    out = Conv3D(nb_filter1, (1, 1, 1), name=conv_name_base + '2a', strides=strides, data_format="channels_last")(input_tensor)
    out = BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(out)
    out = Activation('relu')(out)
    ...
    shortcut = Conv3D(nb_filter3, (1, 1, 1), name=conv_name_base + '1', strides=strides, data_format="channels_last")(input_tensor)
...
def get_resnet50(input_shape, load_weight_path=None) -> Model:
    """
    ...
    out = Conv3D(64, (7, 7, 7), name='conv1', strides=(2, 2, 2), data_format="channels_last")(out)
    ...
    out_class = Conv3D(1, (1, 1, 1), activation="sigmoid", name="out_class_last")(out)
    out_class = Flatten(name="out_class")(out_class)

    out_malignancy = Conv3D(1, (1, 1, 1), activation=None, name="out_malignancy_last")(out)
    out_malignancy = Flatten(name="out_malignancy")(out_malignancy)

    model = Model(inputs=inp, outputs=[out_class, out_malignancy])
    
    if load_weight_path is not None:
        model.load_weights(load_weight_path, by_name=False)    

    model.compile(optimizer=SGD(lr=LEARN_RATE, momentum=0.9, decay=0.0001, nesterov=True),
