...
settings.log = logging.getLogger('step1b_makecubes')
...
def make_pos_annotation_images():
    ...
    for patient_index, csv_file in enumerate(glob.glob(src_dir + "*_annos_pos.csv")):
        ...
        for index, row in df_annos.iterrows():
            ...
            if cube_img.sum() < 5:
                settings.log.info(" ***** Skipping {0} {1} {2}".format(coord_x, coord_y, coord_z))
                continue

            if cube_img.mean() < 10:
                settings.log.info(" ***** Suspicious {0} {1} {2}".format(coord_x, coord_y, coord_z))
    ...
    for patient_index, csv_file in enumerate(glob.glob(src_dir + "*_annos_pos_lidc.csv")):
        ...
        for index, row in df_annos.iterrows():
            ...
            if cube_img.shape != (64, 64, 64):
                settings.log.info(" ***** incorrect shape !!! {0} - {1} {2} {3} ".format(str(anno_index), coord_x, coord_y, coord_z))
                continue
...
def make_candidate_auto_images(candidate_types=[]):
    ...
    for candidate_type in candidate_types:
        ...
        for index, csv_file in enumerate(glob.glob(src_dir + "*_candidates_" + candidate_type + ".csv")):
            ...
            settings.log.info("{0}, patient: {1}, type:{2} ".format(index, patient_id, candidate_type))
            # if not "148229375703208214308676934766" in patient_id:
            #     continue
            df_annos = pandas.read_csv(csv_file)
            if len(df_annos) == 0:
...
if __name__ == "__main__":
    if not os.path.exists(settings.WORKING_DIR + "generated_traindata/"):
        os.mkdir(settings.WORKING_DIR + "generated_traindata/")
